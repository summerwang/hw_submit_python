...
def create_directory(user_input):
	professor = getpass.getuser()
	path_pi = user_input[0]
	head = path_pi
	while head != '/':
...
def create_submit(user_input):
	...
	path_pi += user_input[1]
	course_d = ''.join(path_pi)
	submit_de = course_d + '/submit'
	submit_e = ''.join(submit_de)
	course_d += '/Submissions/'
	submission_d = ''.join(course_d)
	submit_d = submission_d + 'submit.py'
	submit_file = ''.join(submit_d)
	submission_d += user_input[2]
	destination = ''.join(submission_d)
	
	#print(submit_file)
	if os.path.isfile(submit_file):
		print('\'submit\' has been created before for course %s. We will re-create \'submit\' if you simply press [Enter].\nIf you want your students to use \'submit\' created before, enter \'quit\' to quit the program.' % user_input[1])
		reply_de = input('>>>')
		if  reply_de == 'quit':
			print('You entered \'quit\' and will quit the program ...')
			sys.exit(201)
		else:
			print('\'submit\' is re-created.')
			os.remove(submit_file)

	#Get the current path where pi_submit.py is located 
	path_pisubmit = os.getcwd()
	shutil.copyfile(path_pisubmit + '/submit.py', submit_file)
	file = open(submit_file, 'w')
#	file.write("""# This script is used by students to submit assignments into designated submission directory.
# Every use by the student deletes any previous submissions.
# Use:""")
#	file.write("%s" %(submit_e))
#	file.write("\n# The size limit of one assignment submitted by each student is " + user_input[3] + " MB")
#	file.write("""\ndef submit_homework():
#""")
	file.close()
